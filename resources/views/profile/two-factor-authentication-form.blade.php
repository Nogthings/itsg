<x-jet-action-section>
    ...
    <x-slot name="description">
        {{ __('Agraga seguridad extra a tu cuenta habilitando la autencitación de dos factores.') }}
    </x-slot>
    ...
    <x-slot name="content">
        <h3 class="text-lg font-medium text-gray-900 dark:text-gray-50">
            @if ($this->enabled)
                ...
                @endif
            @else
                {{ __('No tienes activada la autenticacion de dos factores.') }}
            @endif
        </h3>
        ...
        @if ($this->enabled)
            @if ($showingQrCode)
                <div class="mt-4 max-w-xl text-sm text-gray-600">
                    <p class="font-semibold text-gray-600 dark:text-gray-50">
                        @if ($showingConfirmation)
                            {{ __('Para terminar de habilitar la autenticación de dos factores, escanee el siguiente código QR usando la aplicación de autenticación de su teléfono (Como el Autenticador de Google o Microsoft Authenticator) o ingrese la clave de configuración y proporcione el código OTP generado.') }}
                        @else
                ...
                @if ($showingRecoveryCodes)
                    <x-jet-confirms-password wire:then="regenerateRecoveryCodes">
                        <x-jet-secondary-button class="mr-3">
                            {{ __('Generar nuevos codigos de recuperación') }}
